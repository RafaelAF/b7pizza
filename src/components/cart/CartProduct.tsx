...
export const CartProduct = ({data}: Props) => {
    ...
    let product = products.products.find(p => p.id === data.productId) 
    ...
    return (
        <div className="flex items-center gap-3">
            ...
            <div className="flex items-center bg-secondary p-2 rounded-md">
                <Button onClick={handleMinusClick} size="sm" variant={qt == 1 ? "ghost" : "default"}>-</Button>
                <div className="mx-3">{qt}</div>
                <Button onClick={handlePlusClick} size="sm" variant={"default"}>+</Button>
            </div>
        </div>
    )
}